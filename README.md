...
To get started just install dependencies.  
`npm i`  
For first time setup simply open  
`install-ableton-js.command`  
(this is just for mac -- do manual setup on other platforms).

Then to run on a regular basis either open `launch.command` (mac), or run `npm start` in a terminal.  
I may bundle some binary releases in future.
...
This looks for tracks with `[BUSX]` (where X is 1-8) in the name and maps them to the 2x4 buttons on the bottom. These buttons activate indicate 'bus selection' (I may expand this concept in future - e.g. auto-map faders to bus volume).

When a bus is selected, macro buttons on top right begin acting as a looper controls (if an instance of Looper is detected on said bus).
